...
configure_logging()
logger = logging.getLogger("astraflow.mcp_llm_gateway")

app = FastAPI(title="AstraFlow MCP LLM Gateway", version="0.1.0")
...
def _build_stream_result(payload: LlmGatewayRequest, model: str, summary: str, reasoning: str) -> AgentResult:
    return AgentResult(
        ...
        process_steps=[
            f"MCP LLM Gateway 选择 Provider：{payload.provider}",
            f"调用模型：{model}",
        ],
